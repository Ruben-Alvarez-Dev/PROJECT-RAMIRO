...
export class RamiroMCPServer {
    ...
    this.tools = [
      {
        name: 'ramiro_voice',
        description:
          'Start or stop a realtime voice session with Ramiro. Use for study conversations, exam practice, or Q&A.',
        inputSchema: {
          type: 'object',
          properties: {
            action: {
              type: 'string',
              enum: ['start', 'stop'],
              description: 'Start or stop the voice session',
            },
            mode: {
              type: 'string',
              enum: ['always-on', 'push-to-talk'],
              description: 'Microphone mode',
              default: 'always-on',
            },
            model_mode: {
              type: 'string',
              enum: ['auto', 'omni', 'pro', 'both'],
              description: 'Model routing mode',
              default: 'auto',
            },
      ...
      },
      {
        name: 'ramiro_video',
        description:
          'Start or stop a realtime video session with Ramiro. Supports camera, screen, window, and OBS sources (up to 4 simultaneous).',
        inputSchema: {
          type: 'object',
          properties: {
            action: {
              type: 'string',
              enum: ['start', 'stop'],
              description: 'Start or stop the video session',
            },
            sources: {
              type: 'array',
              items: { type: 'string', enum: ['camera', 'screen', 'window', 'obs'] },
              description: 'Video sources to capture',
            },
            model_mode: {
              type: 'string',
              enum: ['auto', 'omni', 'pro', 'both'],
      ...
      },
      {
        ...
        description:
          "Search, add, or manage Ramiro's knowledge base. The knowledge base contains the opposition syllabus, study notes, and indexed documents.",
        inputSchema: {
          type: 'object',
          properties: {
            action: {
              type: 'string',
              enum: ['search', 'add', 'recall', 'status', 'ingest'],
            ...
            file_path: {
              type: 'string',
              description: 'Path to file for ingestion (for ingest action)',
            },
          },
          required: ['action'],
        },
      },
      {
        name: 'ramiro_memory',
        description:
          "Manage Ramiro's persistent memory: save insights, recall past sessions, consolidate learnings. Memory persists between sessions.",
        inputSchema: {
          type: 'object',
          properties: {
            action: {
              type: 'string',
              enum: ['save', 'recall', 'consolidate', 'list', 'delete'],
      ...
      },
      {
        name: 'ramiro_status',
        description:
          "Get Ramiro's current status: active session info, pipeline state, context usage, stagnation state, topic focus, and model routing.",
        inputSchema: {
          type: 'object',
          properties: {
            detail: {
  ...
  async handleToolCall(call: MCPToolCall): Promise<MCPToolResult> {
    try {
      ...
      this.logger.error('MCP tool error', error instanceof Error ? error : new Error(message), {
        tool: call.name,
      });
  ...
  private async handleKnowledge(args: Record<string, unknown>): Promise<MCPToolResult> {
    ...
    switch (action) {
      ...
      default:
        return {
          content: [{ type: 'text', text: `Unknown knowledge action: ${action}` }],
          isError: true,
        };
    }
  }
  ...
  private async handleMemory(args: Record<string, unknown>): Promise<MCPToolResult> {
    ...
    switch (action) {
      ...
      case 'delete': {
        ...
        return {
          content: [
            { type: 'text', text: deleted ? `Deleted: ${args.name}` : `Not found: ${args.name}` },
          ],
        };
      }
      default:
